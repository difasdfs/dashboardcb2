...
def loginpage(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            login(request, user)
            return redirect('indexcrisbarmetro')
        else:
...
def index(request):
    ...
    revenue_sharing = format_rupiah.main(total_sales*0.2, total_penjualan=True)
    ...
    return render(request, 'investormetro/index.html', context)