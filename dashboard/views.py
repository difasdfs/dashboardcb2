...
def manager(request):
    ...
    periode_kerja = PeriodeKerja.objects.get(pk=4)
    c = Complaint.objects.all()
    c = c.filter(tanggal__range=[periode_kerja.awal_periode, periode_kerja.akhir_periode])
    # HITUNG JUMLAH COMPLAINT CABANG
    # ANTAPANI
    complaint_antapani = c.filter(cabang="Antapani")
    total_complaint_antapani = len(complaint_antapani)

    kualitas_produk_antapani = complaint_antapani.filter(jenis="Kualitas Produk")
    kualitas_produk_antapani = len(kualitas_produk_antapani)

    kebersihan_antapani = complaint_antapani.filter(jenis="Kebersihan")
    kebersihan_antapani = len(kebersihan_antapani)

    pelayanan_antapani = complaint_antapani.filter(jenis="Pelayanan")
    pelayanan_antapani = len(pelayanan_antapani)

    tidak_lengkap_antapani = complaint_antapani.filter(jenis="Tidak Lengkap")
    tidak_lengkap_antapani = len(tidak_lengkap_antapani)

    # CISITU
    complaint_cisitu = c.filter(cabang="Cisitu")
    total_complaint_cisitu = len(complaint_cisitu)

    kualitas_produk_cisitu = complaint_cisitu.filter(jenis="Kualitas Produk")
    kualitas_produk_cisitu = len(kualitas_produk_cisitu)

    kebersihan_cisitu = complaint_cisitu.filter(jenis="Kebersihan")
    kebersihan_cisitu = len(kebersihan_cisitu)

    pelayanan_cisitu = complaint_cisitu.filter(jenis="Pelayanan")
    pelayanan_cisitu = len(pelayanan_cisitu)

    tidak_lengkap_cisitu = complaint_cisitu.filter(jenis="Tidak Lengkap")
    tidak_lengkap_cisitu = len(tidak_lengkap_cisitu)

    # JATINANGOR
    complaint_jatinangor = c.filter(cabang="Jatinangor")
    total_complaint_jatinangor = len(complaint_jatinangor)

    kualitas_produk_jatinangor = complaint_jatinangor.filter(jenis="Kualitas Produk")
    kualitas_produk_jatinangor = len(kualitas_produk_jatinangor)

    kebersihan_jatinangor = complaint_jatinangor.filter(jenis="Kebersihan")
    kebersihan_jatinangor = len(kebersihan_jatinangor)

    pelayanan_jatinangor = complaint_jatinangor.filter(jenis="Pelayanan")
    pelayanan_jatinangor = len(pelayanan_jatinangor)

    tidak_lengkap_jatinangor = complaint_jatinangor.filter(jenis="Tidak Lengkap")
    tidak_lengkap_jatinangor = len(tidak_lengkap_jatinangor)

    # METRO
    complaint_metro = c.filter(cabang="Metro")
    total_complaint_metro = len(complaint_metro)

    kualitas_produk_metro = complaint_metro.filter(jenis="Kualitas Produk")
    kualitas_produk_metro = len(kualitas_produk_metro)

    kebersihan_metro = complaint_metro.filter(jenis="Kebersihan")
    kebersihan_metro = len(kebersihan_metro)

    pelayanan_metro = complaint_metro.filter(jenis="Pelayanan")
    pelayanan_metro = len(pelayanan_metro)

    tidak_lengkap_metro = complaint_metro.filter(jenis="Tidak Lengkap")
    tidak_lengkap_metro = len(tidak_lengkap_metro)
    
    # SUKAPURA
    complaint_sukapura = c.filter(cabang="Sukapura")
    total_complaint_sukapura = len(complaint_sukapura)

    kualitas_produk_sukapura = complaint_sukapura.filter(jenis="Kualitas Produk")
    kualitas_produk_sukapura = len(kualitas_produk_sukapura)

    kebersihan_sukapura = complaint_sukapura.filter(jenis="Kebersihan")
    kebersihan_sukapura = len(kebersihan_sukapura)

    pelayanan_sukapura = complaint_sukapura.filter(jenis="Pelayanan")
    pelayanan_sukapura = len(pelayanan_sukapura)

    tidak_lengkap_sukapura = complaint_sukapura.filter(jenis="Tidak Lengkap")
    tidak_lengkap_sukapura = len(tidak_lengkap_sukapura)

    # SUKABIRUS
    complaint_sukabirus = c.filter(cabang="Sukabirus")
    total_complaint_sukabirus = len(complaint_sukabirus)

    kualitas_produk_sukabirus = complaint_sukabirus.filter(jenis="Kualitas Produk")
    kualitas_produk_sukabirus = len(kualitas_produk_sukabirus)

    kebersihan_sukabirus = complaint_sukabirus.filter(jenis="Kebersihan")
    kebersihan_sukabirus = len(kebersihan_sukabirus)

    pelayanan_sukabirus = complaint_sukabirus.filter(jenis="Pelayanan")
    pelayanan_sukabirus = len(pelayanan_sukabirus)

    tidak_lengkap_sukabirus = complaint_sukabirus.filter(jenis="Tidak Lengkap")
    tidak_lengkap_sukabirus = len(tidak_lengkap_sukabirus)

    # SUKAJADI
    complaint_sukajadi = c.filter(cabang="Sukajadi")
    total_complaint_sukajadi = len(complaint_sukajadi)

    kualitas_produk_sukajadi = complaint_sukajadi.filter(jenis="Kualitas Produk")
    kualitas_produk_sukajadi = len(kualitas_produk_sukajadi)

    kebersihan_sukajadi = complaint_sukajadi.filter(jenis="Kebersihan")
    kebersihan_sukajadi = len(kebersihan_sukajadi)

    pelayanan_sukajadi = complaint_sukajadi.filter(jenis="Pelayanan")
    pelayanan_sukajadi = len(pelayanan_sukajadi)

    tidak_lengkap_sukajadi = complaint_sukajadi.filter(jenis="Tidak Lengkap")
    tidak_lengkap_sukajadi = len(tidak_lengkap_sukajadi)

    # UNJANI
    complaint_unjani = c.filter(cabang="Unjani")
    total_complaint_unjani = len(complaint_unjani)

    kualitas_produk_unjani = complaint_unjani.filter(jenis="Kualitas Produk")
    kualitas_produk_unjani = len(kualitas_produk_unjani)

    kebersihan_unjani = complaint_unjani.filter(jenis="Kebersihan")
    kebersihan_unjani = len(kebersihan_unjani)

    pelayanan_unjani = complaint_unjani.filter(jenis="Pelayanan")
    pelayanan_unjani = len(pelayanan_unjani)

    tidak_lengkap_unjani = complaint_unjani.filter(jenis="Tidak Lengkap")
    tidak_lengkap_unjani = len(tidak_lengkap_unjani)

    # QUERYNYA
    antapani = [kualitas_produk_antapani, kebersihan_antapani, pelayanan_antapani, tidak_lengkap_antapani, total_complaint_antapani]
    cisitu = [kualitas_produk_cisitu, kebersihan_cisitu, pelayanan_cisitu, tidak_lengkap_cisitu, total_complaint_cisitu]
    jatinangor = [kualitas_produk_jatinangor, kebersihan_jatinangor, pelayanan_jatinangor, tidak_lengkap_jatinangor, total_complaint_jatinangor]
    metro = [kualitas_produk_metro, kebersihan_metro, pelayanan_metro, tidak_lengkap_metro, total_complaint_metro]
    sukapura = [kualitas_produk_sukapura, kebersihan_sukapura, pelayanan_sukapura, tidak_lengkap_sukapura, total_complaint_sukapura]
    sukabirus = [kualitas_produk_sukabirus, kebersihan_sukabirus, pelayanan_sukabirus, tidak_lengkap_sukabirus, total_complaint_sukabirus]
    sukajadi = [kualitas_produk_sukajadi, kebersihan_sukajadi, pelayanan_sukajadi, tidak_lengkap_sukajadi, total_complaint_sukajadi]
    unjani = [kualitas_produk_unjani, kebersihan_unjani, pelayanan_unjani, tidak_lengkap_unjani, total_complaint_unjani]
